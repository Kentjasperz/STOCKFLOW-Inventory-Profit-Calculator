...
void viewInventory() {
    ...
    cout << "\n--- INVENTORY LIST ---\n";
    cout << left<< setw(15) << "Item" << setw(13) << "Initial Qty" << setw(13) <<
    "Current Qty" << setw(12) << "Price" << setw(12) << "Stock" << "\n";
    cout << string(60, '-') << "\n";
    
    for (int i = 0; i < inventory.size(); i++) {
        cout  << "[" << i+1 << "]" << left << setw(15) << inventory[i].name 
        << setw(13) << inventory[i].initial_qty << setw(13) << inventory[i].current_qty
...
void totalProfit() {
    ...
    for (int i = 0; i < inventory.size(); i++) {
        ...
        if (sold > 0) {
            ...
            // Items were restocked - no profit/loss from this
            cout << inventory[i].name << ": P0.00 (restocked: " 
                 << abs(sold) << " units added)\n";
        } else {
            // No change in quantity
            cout << inventory[i].name << ": P0.00 (no sales)\n";
        }
    }
    
    cout << string(40, '-') << "\n";
    cout << "TOTAL PROFIT: P" << fixed << setprecision(2) << total << "\n";
}
